...
function populateCat(rc_conn, wiki_conn) {
  var result = rc_conn.querySync("select rc_id, rc_page_id from recentchanges where rc_ns=0 and rc_handled=0 limit 1");
  var rows = result.fetchAllSync();
  var rcId, pageId;
  if(rows.length > 0) {
    rcId = rows[0].rc_id;
